...
export function createBtns() {
  ...
  const worldBtnImg = createImgElement("world-btn-img", svgEarth1, "Earth Icon", "");
  const tempScaleBtn = createBtnElement("temp-scale-btn", "", "round-btn");
  tempScaleBtn.value = "F";
  const tempScaleImg = createImgElement("temp-scale-img", svgF, "Celsius or Fahrenheit Icon", "");
  const usaBtn = createBtnElement("usa-btn", "", "round-btn");
  const usaBtnImg = createImgElement("usa-btn-img", svgUSA, "United States Icon", "");
